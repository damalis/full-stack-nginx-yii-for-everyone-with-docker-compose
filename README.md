...
- [yiiframework (php-fpm)](https://hub.docker.com/r/yiisoftware/yii2-php)
...
download with
```
git clone https://github.com/damalis/full-stack-nginx-yiiframework-for-everyone-with-docker-compose.git
```
Open a terminal and `cd` to the folder in which `docker-compose.yml` is saved and run:

```
cd full-stack-nginx-yiiframework-for-everyone-with-docker-compose
chmod +x install.sh
./install.sh
```
...
```
docker volume create --driver local --opt type=none --opt device=/home/ubuntu/full-stack-nginx-yiiframework-for-everyone-with-docker-compose/certbot --opt o=bind certbot-etc
```
